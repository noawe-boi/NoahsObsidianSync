...
\begin{tikzpicture}
	...
	\legend{Raw data (outliers removed), Linear Regression}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		\begin{axis}[xmin=0, xmax=1.1, ymin=0.322, ymax=0.78, axis lines=left, scale mode=stretch to fill, width=0.8\paperwidth, height=6cm, xtick distance=0.1, ytick distance=0.2, legend pos=north west, ylabel=$E_\textrm{cell}$]
			...
			\addplot[no markers, blue, domain=0:1.1, samples=100, xlabel=] {0.0820068315129334*ln(x)+0.581619941927291};
			\node[anchor=south west,inner ysep=4.6cm, inner xsep=6.5cm, color=blue]  {$y=0.5816+0.08201\cdot\ln\left(x\right)$};
			\node[anchor=south west,inner ysep=4.1cm, inner xsep=6.5cm, color=blue]  {$R^{2}=0.4082$};
			
			
			\legend{Raw data (outliers removed), Logarithmic Regression}
		\end{axis}
	\end{tikzpicture}
...
\begin{itemize}
	...
	\item Temperature and pressure unknown on day of experimentation. This means that the Nernst equation cannot account for them. Therefore difference in between Nernst equation and actual results n
	\item increasing number of trials would have allowed voltage drop off to be found and also would have improved reliability after outliers removed. 
\end{itemize}
