...
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
		\begin{axis}[xmin=0, xmax=1.1, ymin=0.322, ymax=0.78, axis lines=left, scale mode=stretch to fill, width=0.8\paperwidth, height=6cm, xtick distance=0.1, ytick distance=0.2, legend pos=north west, ylabel=$E_\textrm{cell}$]
			\addplot[only marks, mark=*, color=black]
			coordinates {
				(1, 0.680666666666667)
				(0.85, 0.591666666666667)
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[xlabel=${ \left[Cu^{2+}\right] }$, ylabel=$E_{\textrm{cell}}$, width=0.8\paperwidth, height=6cm, xmin=0, ymin=0, xmax=1.1]
			\addplot
			[
			samples=2000, smooth
			]
		...
		\addplot[no markers, blue, samples=2] {(0.236792*x) + 0.37479};
		\legend{Nerenst Equation, Experimental LR}
		\end{axis}
...
\begin{table}[]
	\begin{tabular}{|l|l|l|}
		...
		\begin{tabular}[c]{@{}l@{}}Floating voltage on multimeter in between trials. \\ Voltage fluxuation throughout trials\end{tabular}         & \begin{tabular}[c]{@{}l@{}}Multimeter wires in contact with \\ puddle of electrolyte at \\ intermediate connection point \\ between meter and probes. \\ A minor redox reaction may \\ have been ocuring on the bench, \\ affecting readings.\end{tabular}                   & \begin{tabular}[c]{@{}l@{}}Isolate multimeter from experimental setup and ensure \\ probes are only connected to electrodes.\end{tabular}                                                                                     \\ \hline
		\begin{tabular}[c]{@{}l@{}}Inconsistient voltage readings and high electrolyte \\ consumption over relativley short readings\end{tabular} & \begin{tabular}[c]{@{}l@{}}Alligator clips of unknown material \\ submerged in half cell \\ electrolyte solution. Likely \\ dissociating and affecting half cell \\ composition unintentionally.\end{tabular}                                                                & \begin{tabular}[c]{@{}l@{}}Alter shape of electrodes to a portion of it extends \\ beyond the edge of the beaker so that aligator clips \\ can be attathced without being at risk of contacting \\ electrolytes.\end{tabular} \\ \hline
		Large voltage drops between trials                                                                                                        & \begin{tabular}[c]{@{}l@{}}Inonsistient measurement times and \\ gaps between trials. During some trials,\\ the multimeter disconnected midway \\ through measurement and when reconnected \\ the voltage reading was significantly lower \\ than other trials.\end{tabular} & \begin{tabular}[c]{@{}l@{}}Perform all steps at set time intervals. Take care to \\ ensure setup remains the same throughout all trials.\end{tabular}                                                                         \\ \hline
		Consistiently lower voltages than expected                                                                                                & \begin{tabular}[c]{@{}l@{}}Unknown. Possibly resistance due to small salt bridge or \\ poor wiring, oxide layer on electrodes, incorrect electrolyte concentrations.\end{tabular}                                                                                            & Follow procedure with more care. Use more suitable equipment.                                                                                                                                                                 \\ \hline
	\end{tabular}
\end{table}
